...
async def button_handler(update: Update, context: CallbackContext):
    ...
    # Платежи
        ...
        if user_id in ADMIN_IDS:
            from .payment_handlers import start_payment
            return await start_payment(update, context)
    
    elif data.startswith("get_payment_report_"):
        if user_id in ADMIN_IDS:
...
async def send_payment_report(update: Update, context: CallbackContext, display_name: str):
    ...
    try:
        ...
        from ...utils.payment_utils import normalize_date
        from openpyxl import Workbook
        from io import BytesIO
        from datetime import datetime
        import pandas as pd
        ...
        for (spreadsheet_id, sheet_name), records in sheets.items():
            ...
            ws = wb.create_sheet(title=f"{sheet_name[:30]}")  # Ограничиваем название листа
            
            # Заголовки
            headers = ['ID', 'Ամսաթիվ', 'Մատակարար', 'Ուղղություն', 'Նկարագրություն', 'Գումար']
            ws.append(headers)
            ...
            # Добавляем строку итога
            ws.append(['', '', '', '', 'Իտոգ:', total_amount])
            
            # Получаем платежи для этого листа
            ...
            all_summaries.append({
                ...
                'Ծախս': total_amount,
                'Վճար': total_payments,
                'Մնացորդ': total_amount - total_payments
            })

        # Создаем итоговый лист
        if all_summaries:
            summary_ws = wb.create_sheet(title="Ամփոփ հաշվետվություն", index=0)
            summary_headers = ['Աղյուսակ', 'Թերթիկ', 'Ընդհանուր ծախս', 'Ընդհանուր վճար', 'Մնացորդ']
            ...
            for summary in all_summaries:
                ...
                total_expense += summary['Ծախս']
                total_payment += summary['Վճար']
            
            # Итоговая строка
            summary_ws.append(['', 'Ընդհանուր:', total_expense, total_payment, total_expense - total_payment])
        ...
        # Отправляем файл
        filename = f"payment_report_{display_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        
        await query.message.reply_document(
            document=file_buffer,
            filename=filename,
            caption=f"� Վճարային հաշվետվություն {display_name}-ի համար\n"
                   f"📅 Ստեղծվել է: {datetime.now().strftime('%Y-%m-%d %H:%M')}"
        )
        
        await query.edit_message_text(
            f"✅ Վճարային հաշվետվությունը ուղարկված է {display_name}-ի համար",
            reply_markup=create_back_to_menu_keyboard()
        )
        
        from ...utils.config_utils import send_to_log_chat
        await send_to_log_chat(context, f"Ստեղծվել է վճարային հաշվետվություն {display_name}-ի համար")
        
    except Exception as e:
        logger.error(f"Ошибка создания отчета для {display_name}: {e}")
        await query.edit_message_text(
            f"❌ Վճարային հաշվետվություն ստեղծելու սխալ: {e}",
            reply_markup=create_back_to_menu_keyboard()
        )
