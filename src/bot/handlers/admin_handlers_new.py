...
async def initialize_sheets_command(update: Update, context: CallbackContext):
    ...
    try:
        # Здесь должна быть логика инициализации всех листов
        # Пока что заглушка
        await update.message.reply_text("✅ Բոլոր աղյուսակները հաջողությամբ մշակված են:")
        await send_to_log_chat(context, "✅ Կատարվել է /initialize_sheets հրամանը")
...
async def set_report_sheet_handler(update: Update, context: CallbackContext):
    ...
    # Заглушка для обработчика настройки отчетов
    await update.message.reply_text(
        "📊 Հաշվետվությունների կարգավորում...\n"
        "Այս գործառույթը դեռ չի իրականացված:",
